...
class OptimizationMethod():
    ...
    def newton_with_inexact_line_serach(self,x_init,step_size=0.00001,stopping_criteria=1e-20, maxIters=50):
        return self.Newton_help_method(x_init, 'inexact_line_search',step_size,stopping_criteria,maxIters)
    ...
    def Newton_help_method(self,x_init,line_search,step_size=0.00001,stopping_criteria=1e-20, maxIters=50):
        ...
        while (np.all(g>stopping_criteria) or iteration<maxIters):
            ...
            if line_search == 'exact_line_search':
                a = so.minimize_scalar(lambda a : self.opt_problem.function_value(x+a*s)).x    
...
def plot_rosenbrock_function():
    ...
    contour = plt.contour(X, Y, Z ** 0.33, contour_levels, cmap='viridis')
...
def main():
    ...
    # Use a numerical optimization algorithm to find the minimum
    result = opt.minimize(rosenbrock_function, x, method='BFGS')

    # The solution will be in result.x
    solution = result.x
